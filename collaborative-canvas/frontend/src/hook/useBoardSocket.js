...
import { addThread, updateThread } from "../redux/threadsSlice";
...
const useBoardSocket = () => {
  ...
  useEffect(() => {
    ...
    const handlers = {
      ...
      iterationImageUpdate: ({ boardId, iterationId, prompt, imageUrl }) => {
        dispatch(
          updateIterationPartial({
            boardId,
            iterationId,
    ...
    dispatchWithMeta,
    dispatch,
    imgUploadProgressHandler,
    imgGenProgressHandler,
  ]);
};

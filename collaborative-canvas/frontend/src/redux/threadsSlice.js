...
export const { setThreads, addThread, addThreads, updateThread, removeThread } =
...
const countChildren = (parentThreads, allThreads) => {
  return parentThreads.map((parent) => ({
    ...parent,
    childrenCount: allThreads?.filter(
      (child) => child.parentId === parent._id && child.isResolved === false
    ).length,
  }));
};
...
const populateChildren = (parentThread, allThreads) => {
  ...
  return {
    ...
    children: allThreads
      ?.filter(
        (child) =>
          child.isResolved === false && child.parentId === parentThread._id
...
export const selectParentThreadsByImage = (imageId) =>
  createSelector([selectAllThreads], (allThreads) => {
    const parentThreads = allThreads?.filter(
      (thread) =>
        thread.imageId === imageId && !thread.parentId && !thread.isResolved
    );
...
export const selectParentThreadsByKeyword = (keywordId) =>
  createSelector([selectAllThreads], (allThreads) => {
    const parentThreads = allThreads?.filter(
      (thread) =>
        thread.keywordId === keywordId && !thread.parentId && !thread.isResolved
    );
    return countChildren(parentThreads, allThreads);
  });

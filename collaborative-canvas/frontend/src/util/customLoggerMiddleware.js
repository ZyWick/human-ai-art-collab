...
const getActionMeta = (meta) =>
  meta?.id ? ` by ${meta.name} (${meta.id})` : "";
    ...
    if (type.startsWith(`${entity}/`)) {
      if (
        ...
        logAction(
          ...
          actionMeta,
          action,
          [`addKeyword`, `addThread`, `addImage`, `addBoard`].some((suffix) =>
            type.endsWith(suffix)
          )
            ? { label: `New ${entity}:`, data: payload }
            : type.endsWith("updateBoardIterations")
            ? { label: `New Iteration:`, data: payload }
            : null
        );
      } else if (
        ["updateKeyword", "updateThread", "updateBoard"].some((suffix) =>
          type.endsWith(suffix)
        )
      ) {
        ...
        if (prevState) {
          ...
          if (Object.keys(stateChanges).length) {
            logAction(
              `${type} [${Object.keys(stateChanges).join(", ")}]`,
              actionMeta,
              action,
              { label: "State Changes:", data: stateChanges }
            );
          }
        }
      }
...
export const keywordLoggerMiddleware = createLoggerMiddleware("keywords", [
  "votes",
  "downvotes",
  "isSelected",
  "offsetX",
...
export const threadLoggerMiddleware = createLoggerMiddleware("threads", [
  "isResolved",
  "value",
]);
export const boardLoggerMiddleware = createLoggerMiddleware("boards", [
  "name",
  "isStarred",
  "isVoting",
]);
...
export const roomLoggerMiddleware = (store) => (next) => (action) => {
  ...
  if (!["socket/updateDesignDetailsFull", "socket/setRoomName"].includes(type))
    return next(action);
  ...
  const changes = trackedFields.reduce(
    (acc, key) =>
      prevState[key] !== nextState[key]
        ? { ...acc, [key]: { before: prevState[key], after: nextState[key] } }
        : acc,
    {}
  );
